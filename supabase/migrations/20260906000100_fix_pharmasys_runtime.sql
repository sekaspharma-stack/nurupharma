...
-- 1) Identity-backed IDs: keep PostgreSQL identity columns intact.
-- Do NOT replace their identity defaults with manually-created sequences:
-- PostgreSQL rejects SET DEFAULT on an identity column.
-- If rows already exist, simply advance the native identity sequences.
SELECT setval(pg_get_serial_sequence('ticket_lignes', 'id'),
              COALESCE((SELECT MAX(id) FROM ticket_lignes), 0) + 1, false);

SELECT setval(pg_get_serial_sequence('mouvements_stock', 'id'),
              COALESCE((SELECT MAX(id) FROM mouvements_stock), 0) + 1, false);

SELECT setval(pg_get_serial_sequence('inventaires', 'id'),
              COALESCE((SELECT MAX(id) FROM inventaires), 0) + 1, false);

SELECT setval(pg_get_serial_sequence('controles_inventaire', 'id'),
              COALESCE((SELECT MAX(id) FROM controles_inventaire), 0) + 1, false);

-- 2) Pharmacy settings: keep the existing identity ID and enforce one row per tenant.
ALTER TABLE parametres_pharmacie ALTER COLUMN id TYPE bigint;
SELECT setval(pg_get_serial_sequence('parametres_pharmacie', 'id'),
              COALESCE((SELECT MAX(id) FROM parametres_pharmacie), 0) + 1, false);
CREATE UNIQUE INDEX IF NOT EXISTS uq_parametres_pharmacie_tenant ON parametres_pharmacie(tenant_id);

-- 3) Inventaire/user IDs: these legacy bigint columns are NOT identity columns,
-- so give them proper database-owned sequences instead of Date.now()-style IDs.
